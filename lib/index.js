 ...
 * @arg {Boolean|Number} [input.typing] - Indicates whether the message should be sent with simulated typing. Based on message length.
 */
...
DCP.ban = function(input, callback) {
    ...
    if (input.lastDays = Number(input.lastDays)) {
        opts.lastDays = Math.max(Math.min(input.lastDays, 7),1);
	}
...
DCP.createChannel = function(input, callback) {
	var client = this, payload = {
		name: input.name,
		type: (['text', 'voice', 'category'].indexOf(input.type) < 0) ? 'text' : input.type, //(input.type === "category" ? 4 : input.type),
 ...
 * @arg {Object} input
 * @arg {Snowflake} input.channelID
 * @arg {String} [input.name] - The name of the channel.
 ...
 * @arg {boolean} [input.nsfw] - Toggles NSFW.
 * @arg {Snowflake} [input.parentID] - [Channel Only] The channel's parent category.
 */
...
DCP.editChannelPermissions = function(input, callback) { //Will shrink this up later
	...
	allowed_values = [0, 4, 28].concat((channel.type === 'text' || channel.type === 0 || channel.type === 'category' || channel.type === 4) ?
	[6, 10, 11, 12, 13, 14, 15, 16, 17, 18] :
	[]).concat((channel.type === 'voice' || channel.type === 2 || channel.type === 'category' || channel.type === 4) ? 
...
function handleWSMessage(data, flags) {
	...
	switch (message.op) {
		...
		case 7:
			...
			break;
		case 9:
			console.log("DISCORD INVALIDATED MY SESSION REEEEEEEEEE");
...
ACBP.playAudioFile = function(location, callback) {
	...
	enc = ChildProc.spawn(selection , [
		'-i', location,
		'-f', 's16le',
		//'-af', 'loudnorm=I=-24:TP=-1:LRA=1:dual_mono=true',		
		'-af', 'volume=-10dB',
		'-ar', '48000',
...
function prepareAudio(ACBI, readableStream, cnt, wasPaused) {
	if(!ACBI._paused){
		...
		return setTimeout(function() {
			sendAudio(ACBI, data);
			prepareAudio(ACBI, readableStream, cnt + 1, wasPaused);
...
function createAudioEncoder(ACBI, encoder) {
	...
	enc = ACBI._systemEncoder = ChildProc.spawn(encoder, [
		...
		'-sample_fmt', 's16',
		'-vbr', 'off',
		'-compression_level', '0', //10 default
		//'-af', 'loudnorm=I=-24:TP=-1:LRA=1:dual_mono=true',
		'-af', 'volume=-10dB',
		'-ar', '48000',
		'-ac', ACBI.audioChannels,
	...
	enc.stdout.on('readable', function() {
		...
		ACBI._readable = true;
		send(ACBI._voiceSession.ws.connection, Payloads.VOICE_SPEAK(ACBI._paused | 0));
		ACBI._startTime = new Date().getTime();
		prepareAudio(ACBI, enc.stdout, 1, ACBI._paused);
	});
}
...
function Channel(client, server, data) {
	var channel = this;
	channel.members = {};
	channel.permissions = { user: {}, role: {} };
	channel.guild_id = server.id;
	copyKeys(data, channel, ['permission_overwrites', 'emojis']);
	...
	data.permission_overwrites.forEach(function(p) {
		...
		channel.permissions[type][p.id] = {allow: p.allow, deny: p.deny};
	}, channel);
	if(Number(channel.type)){
		channel.type = (['text','text','voice','text','category'])[Number(channel.type)];
	}
	return channel;
}
